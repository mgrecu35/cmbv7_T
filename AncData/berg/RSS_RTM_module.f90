...
! [MW 2012] Table 2 -- files also available at
! /home/dduncan/RSS_RTM/external_files/
! character(len=200), parameter     :: file_coeffs_wind_isotropic = '/PANFS/user/home/smunchak/gmi_emis/LUTs/berg/finetune_emiss_wind.dat' 

! [MW 2012] Tables 3 + 4
! character(len=200), parameter     :: file_coeffs_wind_direction = '/PANFS/user/home/smunchak/gmi_emis/LUTs/berg/fit_emiss_phir_wind.dat' 

! [MW 2012] Tables 8, 9, 10, 11, 12
! character(len=200), parameter     :: file_coeffs_sctterm        = '/PANFS/user/home/smunchak/gmi_emis/LUTs/berg/mk_scatterm_table_all.dat' 

! [MW 2012] eq. (15) for fast computation
! character(len=200), parameter     :: file_em0_ref_freq_sst      = '/PANFS/user/home/smunchak/gmi_emis/LUTs/berg/em0_ref_freq_sst.dat' 
...
implicit none
    ...
    emiss(1)=dot_product(LUT%acoef(:,1,ifreq),xmea) 
    emiss(2)=dot_product(LUT%acoef(:,2,ifreq),xmea) 
...
implicit none
    ...
    do istoke=1,nstoke
        aharm(1,istoke)=sst_fac(istoke)*dot_product(xmea,LUT%bcoef(:,1,istoke,ifreq))
        aharm(2,istoke)=sst_fac(istoke)*dot_product(xmea,LUT%bcoef(:,2,istoke,ifreq))
    enddo  !istoke
...
implicit none
    ...
    sst_fac=1 + dot_product(LUT%rcoef(:,ipol,ifreq),xmea)
...
implicit none
    ...
    xscat1= &
    a1*b1*(c1*LUT%scatterm(i1,l1,j1,k1,:)+c2*LUT%scatterm(i1,l1,j1,k2,:))+ &
    a1*b2*(c1*LUT%scatterm(i1,l1,j2,k1,:)+c2*LUT%scatterm(i1,l1,j2,k2,:))+ &
    a2*b1*(c1*LUT%scatterm(i2,l1,j1,k1,:)+c2*LUT%scatterm(i2,l1,j1,k2,:))+ &
    a2*b2*(c1*LUT%scatterm(i2,l1,j2,k1,:)+c2*LUT%scatterm(i2,l1,j2,k2,:))

    xscat2= &
    a1*b1*(c1*LUT%scatterm(i1,l2,j1,k1,:)+c2*LUT%scatterm(i1,l2,j1,k2,:))+ &
    a1*b2*(c1*LUT%scatterm(i1,l2,j2,k1,:)+c2*LUT%scatterm(i1,l2,j2,k2,:))+ &
    a2*b1*(c1*LUT%scatterm(i2,l2,j1,k1,:)+c2*LUT%scatterm(i2,l2,j1,k2,:))+ &
    a2*b2*(c1*LUT%scatterm(i2,l2,j2,k1,:)+c2*LUT%scatterm(i2,l2,j2,k2,:))
