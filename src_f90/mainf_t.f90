  ...
  !stop
     ...
     IF (st_1 .GE. 0) THEN
        st_1=readgmi(jobname,f1ctmi1(1:igmi1),ngmi1,gMIData%gmiS1(:,:,ngmi_total+1:ngmimax), &
             ...
             gmiData%SCLon(ngmi_total+1:ngmimax),&
             gmiData%SCLat(ngmi_total+1:ngmimax),&
             gmiData%SCAlt(ngmi_total+1:ngmimax),&
             gmiData%SCYaw(ngmi_total+1:ngmimax),&
             gMIData%S2lon(:,ngmi_total+1:ngmimax),  &
             gMIData%S2lat(:,ngmi_total+1:ngmimax), &
             GMIdata%S1eia(:,ngmi_total+1:ngmimax), &
  ...
  !stop
     ...
     IF (st_3 .GE. 0) THEN
        st_3=readgmi(jobname,f1ctmi3(1:igmi2),ngmi3,gMIData%gmiS1(:,:,ngmi_total+1:ngmimax), &
             ...
             GMIdata%S1eia(:,ngmi_total+1:ngmimax), &
             GMIdata%S2eia(:,ngmi_total+1:ngmimax), &
             GMIData%S1sga(:,ngmi_total+1:ngmimax)) ! SJM
     ENDIF
  endif
...
!...Compute orbit & date data
  ...
  !end old sfc routines
  call readwfract()
  
  !SJM 8/14/20
  !Read in additional lookup tables for clear sky retrieval
   !new LUTS for all sky retrieval
  call read_LUT_sfc_class(month_get)
  !Wes Berg's MonoRTM GMI gas absorption
  lut_file='AncData/berg/MonoRTM_v5.3-GMI.tbl'
  call read_LUTgas_monortm(lut_file)
  lut_file = 'AncData/berg/'
  call read_RSS_tables(lut_file)
  lut_file='AncData/LUT.GMIDPR.clw.tkc.bin'
  call read_LUTclw(lut_file)
  lut_file='AncData/raob_eofs/'
  call read_err_eofs(lut_file)
...
end subroutine mainfort_t

subroutine do_chunkx_t(i,ialg, idir)
  use globalData
  implicit none
